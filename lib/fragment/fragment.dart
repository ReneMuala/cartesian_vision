...
class Fragment {
  ...
  void _expand(BasicPixel entryPixel, double tolerance, Image source){
    ...
    min = max = entryPixel.point;
    while (!fragmentExpansionPoints.isEmpty) {
      currentPoint = fragmentExpansionPoints.removeLast();
      if(currentPoint.x > -1 && currentPoint.y > -1 && currentPoint.x < source.width && currentPoint.y < source.height) {
        x = currentPoint.xi; y = currentPoint.yi;
        currentColor = source.getPixel(x, y);
        if(getAlpha(currentColor) > 0 && colorOperations.compareRGB(entryPixel.color, currentColor) <= tolerance){
  ...
  static List<Fragment> generateFragments({required Image polarizedSource, double tolerance = 0.2}){
